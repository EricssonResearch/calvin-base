...
class PrivateAttribute(object):
    ...
    def exists(self, index):
        ...
        """
        return self._node.attributes.has_private_attribute(index)
    
    def get(self, index):
        """
            Returns value of private attribute index, or {}
            if no such attribute. Returns None on error
        """
        return self._node.attributes.get_private(index)


def register(node, actor):
    return PrivateAttribute(node, actor)
