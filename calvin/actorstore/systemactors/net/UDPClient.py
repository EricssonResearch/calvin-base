...
class UDPClient(Actor):
    """
    Etablish an UDP connection and forward all tokens.
    Any recevied data on the UDP connection is forwarded according to protocol.
    ...
    def connect(self):
        self.cc = self['socket'].connect(self.address, self.port, connection_type="UDP")
    ...
    def new_connection(self, control):
        ...
        if protocol != 'udp':
            _log.warn("Protocol '%s' not supported" % (protocol,))
        else:
