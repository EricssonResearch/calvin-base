...
class DynamicHBar(Actor):
    ...
    top_to_bottom: If set to false the new values will roll in bottom instead
    ...
    @manage(['params', 'dimension', 'top_to_bottom', 'max_req_in_progress', 'labels', 'values'])
    def init(self, chart_param={}, dimension=8, top_to_bottom=False, max_req_in_progress=5):
    ...
    def send_request(self, label, value):
        ...
        if self.top_to_bottom:
            self.chart_api.set_axes_label([2] + self.labels[::-1])
            self.chart_api.set_chart_dataset(self.values[::-1])
        else:
            self.chart_api.set_axes_label([2] + self.labels)
